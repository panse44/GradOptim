...
Ridge_Train_BGD <- function(train.Y=NULL, train.X=NULL, alpha = 0.001, tolerance = 0.01,lambda=0,nfolds=5,plot=FALSE,SE_estimates=FALSE)
{
...
# Calling the function to estimate weights using Gradient Descent technique for a Ridge Cost
      weight=Ridge_Gradient_Descent_BGD(train.Y = Y,train.X = X,alpha = alpha,tolerance = tolerance,lambda = j)
...
# Final weights will be calculated by training the entire dataset on the optimum value of lambda
    final_weight=Ridge_Gradient_Descent_BGD(train.Y,train.X,alpha = alpha,tolerance = tolerance,lambda = best_lambda)    
...
# Function which calculates the Standard Error of the weights        
    SE=BGD_Ridge_Boot(train.Y,train.X,lambda = best_lambda)
