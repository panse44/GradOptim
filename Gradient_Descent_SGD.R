# A function to build a linear regression model on the given training data by estimating 
# the coefficients using the Stochastic Gradient Descent Optimization technique.

Gradient_Descent_SGD <- function(train.Y=NULL, train.X=NULL, alpha = 0.001, tolerance = 0.01,plot=FALSE,SE_estimates=FALSE,n_Iter=10)
{
...
# Call the SGD_Linear_Boot function if the corresponding argument passed is set as TRUE    
  SE=0
  if(SE_estimates)
  {
    SE=SGD_Linear_Boot(train.Y,train.X,alpha,tolerance)
...
#An inner FOR loop for estimating the coefficients  
      for( i in 1:N)
      {
        temp= t(train.X)[,i]%*%(train.Y[i] - train.X[i,]%*%weight)
